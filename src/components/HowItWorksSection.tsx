...
const HowItWorksSection = () => {
  ...
  return (
    <section ref={ref} className="py-12 md:py-24 px-4 md:px-12 bg-secondary/50">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 max-w-3xl mx-auto">
        {STEPS.map((step, i) => {
          ...
          return (
            ...
            >
              <div className="w-10 h-10 mx-auto mb-2 rounded-xl bg-champagne/40 flex items-center justify-center">
                <Icon className="w-4 h-4 text-warm-gold" strokeWidth={1.8} />
              </div>
              <span className="inline-block mb-1.5 text-[9px] font-body font-bold text-warm-gold bg-champagne/30 px-2 py-0.5 rounded-full uppercase tracking-wider">
                Passo {step.number}
              </span>
              <h3 className="text-xs md:text-base font-display font-semibold text-foreground mb-0.5">{step.title}</h3>
              <p className="text-[11px] text-muted-foreground font-body">{step.description}</p>
            </div>
          );
        })}
      </div>
