      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 max-w-3xl mx-auto">
        {STEPS.map((step, i) => {
          ...
          return (
            <div
              key={step.number}
              className={`relative text-center bg-background rounded-2xl p-5 shadow-card transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
              style={{ transitionDelay: `${i * 120}ms` }}
            >
              <div className="w-12 h-12 mx-auto mb-3 rounded-2xl bg-champagne/40 flex items-center justify-center">
                <Icon className="w-5 h-5 text-warm-gold" strokeWidth={1.8} />
              </div>
              <span className="inline-block mb-2 text-[10px] font-body font-bold text-warm-gold bg-champagne/30 px-2 py-0.5 rounded-full uppercase tracking-wider">
                Passo {step.number}
              </span>
