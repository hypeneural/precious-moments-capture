    ...
    <section ref={ref} id="faq" className="py-14 md:py-24 px-5 md:px-12">
      <div className={`max-w-2xl mx-auto transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        <div className="text-center mb-6">
          <h2 className="text-[1.75rem] md:text-4xl font-display font-semibold text-foreground">
            Dúvidas frequentes
          </h2>
        </div>
        ...
        <Accordion type="single" collapsible className="space-y-3">
          {FAQ_ITEMS.map((item, i) => (
            ...
            >
              <AccordionTrigger className="text-left text-[1.0625rem] md:text-lg font-body font-bold text-foreground py-4 hover:no-underline">
                {item.q}
              </AccordionTrigger>
              <AccordionContent className="text-[1.0625rem] font-body text-muted-foreground pb-4 leading-relaxed">
                {item.a}
              </AccordionContent>
            </AccordionItem>
          ))}
